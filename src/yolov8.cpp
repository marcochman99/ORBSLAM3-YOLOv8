...
std::vector<std::vector<cv::cuda::GpuMat>> YoloV8::preprocess(const cv::cuda::GpuMat &gpuImg) {
    ...
    if (gpuImg.channels() == 1) {
        cv::cuda::cvtColor(gpuImg, rgbMat, cv::COLOR_GRAY2RGB);
    }
    // Check if the image has three channels
    else if (gpuImg.channels() == 3) {
...
std::vector<Object> YoloV8::postProcessSegmentation(std::vector<std::vector<float>>& featureVectors) {
    const auto& outputDims = m_trtEngine->getOutputDims();
    //std::cout<<"outputDims:" <<outputDims <<std::endl;    //CANT PRINT
    ...
    for (int i = 0; i < numAnchors; i++) {
        ...
        if (score > PROBABILITY_THRESHOLD && (label == 0 || label == 2)) {                      //only when probability > thresold and is person or cars
            float x = *bboxesPtr++;
            float y = *bboxesPtr++;
            float w = *bboxesPtr++;
            float h = *bboxesPtr;
            ...
            //int label = maxSPtr - scoresPtr;  //origin
            cv::Rect_<float> bbox;
            bbox.x = x0;
            bbox.y = y0;
            bbox.width = x1 - x0;
    ...
    for (auto& i : indices) {
        ...
        objs.push_back(obj);    //store above obj into objs
        cnt += 1;
    ...
    // Convert segmentation mask to original frame, obtaining seg mask
    if (!masks.empty()) {
        ...
        cv::Mat binaryImage(m_imgHeight ,m_imgWidth , CV_8U, cv::Scalar(0)); 
        

        for (size_t i = 0; i < indices.size(); i++)             //iterate through all masks, Eg. 2 people >> indices.size() = 2
        {
            std::vector<cv::Vec4i> hierarchy;
            cv::Mat dest, mask,cvtmask;                  
            ...
            cv::Mat roith = binaryImage(objs[i].rect);
            objs[i].boxMask.copyTo(roith);
...
void YoloV8::drawObjectLabels(cv::Mat& image, const std::vector<Object> &objects, unsigned int scale) {
    ...
    if (!objects.empty() && !objects[0].boxMask.empty()) {
        ...
        for (const auto& object: objects) {
            ...
            //draw contour for illustration purposes
            cv::drawContours(image, allContours, -1, cv::Scalar(255, 0, 0), 2);
        }
        // Add all the masks to the image
...
int YoloV8::tickoffCheck(float x, float y){
    ...
    for (const auto& contour : allContours){
        ...
        if (distance >= -25){      
        //        if (distance >= -15){            
            // The point is inside or on the contour or too close to the contour
            checkvar = 1;
            break;
        }
        else{
...
///////once for loop is done, gives whether point is inside any of the contours or not 
    if(checkvar == 1){
        return 1;
    }else{
        return 0;
