...
#include<System.h>
     ...
     YoloV8Config config;
    string onnxModelPath = "/home/fyp/ORB_SLAM3-master/Examples/Stereo/yolov8n-seg.onnx";
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    //send points over as arg
    //std::cout<<"first section"<<std::endl;
    for(int i = 0; i < mvKeys.size(); ++i){
        const cv::KeyPoint& kp2send = mvKeys[i];
        int inOut = 0;
        if(yoloV8.tickoffCheck(kp2send.pt.x, kp2send.pt.y) == 1){
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    //     const cv::KeyPoint& kp2send = mvKeys[i];
    //     int inOut = 0;
    //     inOut = yoloV8.tickoffCheck(kp2send.pt.x, kp2send.pt.y);
  ...
  //          rejCnt++;
   ...
   // std::chrono::duration<double, std::nano> timeduration = timeend - timestart;
    ...
    //1201 - 1208   /MP ~2300 matches~200 TEEHEE
    //    for (int i=200; i<mvKeys.size(); ++i){
    //        mvKeys[i] = cv::KeyPoint(-1,-1,-1);
    //    }
